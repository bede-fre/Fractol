...
static void ft_init_val1_mandelbrot(t_values *val, double zoom)
{
	val->fract.x1 = -2.1;
	val->fract.x2 = 0.6;
	val->fract.y1 = -1.2;
...
static void ft_init_val2_mandelbrot(t_values *val, double zoom)
{
    ...
    while (++(val->fract.x) < val->fract.img_x)
	{
		...
		while (++(val->fract.y) < val->fract.img_y)
		{
			...
			if (val->fract.i == val->fract.i_max)
                ...
                ft_fill_px(val, (val->fract.x + var_x), (val->fract.y + var_y), (val->fract.i * 255.0) * (val->fract.i_max));
        }
	}
}
